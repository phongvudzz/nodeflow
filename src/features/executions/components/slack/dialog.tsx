...
const SlackNodeSchema = z.object({
  variableName: z
    .string()
...
export type SlackFormValues = z.infer<typeof SlackNodeSchema>;
...
export const SlackNodeDialog = ({
  open,
  onOpenChange,
  onSubmit,
  defaultValues = {},
}: SlackNodeDialogProps) => {
  const form = useForm<z.infer<typeof SlackNodeSchema>>({
    defaultValues: {
      variableName: defaultValues.variableName || "",
      content: defaultValues.content || "",
      webhookUrl: defaultValues.webhookUrl || "",
    },
    resolver: zodResolver(SlackNodeSchema),
  });

  const watchVariableName = form.watch("variableName") || "mySlack";

  const handleSubmit = (values: z.infer<typeof SlackNodeSchema>) => {
    onSubmit(values);
    onOpenChange(false);
  };
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription>
                    Make sure you have "content" as the key in your payload (not
                    "text")
                  </FormDescription>
                  <FormMessage />
                </FormItem>
